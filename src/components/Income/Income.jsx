...
const Income = ({ incomes, handleDeleteIncome, handleUpdateIncome, ...props}) => {
  ...
  return (
    ...
    <div id="income-table">
      ...
      { incomes.map((income) => (
        ...
        <p id="name-display">{income.name}</p> 
        <p id="money">${ income.amount.toLocaleString() }</p>
        </div>
        </button>
        <div id="buttons">
        <button id="edit-button" onClick={() => handleUpdate(income._id)}><img id="edit-img" src="../src/assets/edit-pencil.png" alt="pen" height="25px" /></button>
        <button id="trash-button" onClick={() => handleDeleteIncome(income._id)}><img id="trash-img" src="../src/assets/trashcan.png" alt="trashcan" height="25px" /></button>
        </div>
        {detailsShow(income)}
        </div>
      ))}
